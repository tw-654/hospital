...
const DoctorDeletePage = () => {
 ...
 // 处理更新排班信息
    const handleUpdateSchedule = async (doctorId: number) => {
        ...
        try {
            ...
            const formatDateTime = (dateTimeStr: string) => {
                ...
                console.log(`时间格式化: ${dateTimeStr} -> ${utcDateTime}`);
                return utcDateTime;
            };

            const requestData = {
                doctorId: doctorId, // 使用 doctorId
                appointmentPrice: Number(scheduleForm.appointment_price), // 修改参数名称
                startTime: formatDateTime(scheduleForm.start_time), // 修改参数名称
                endTime: formatDateTime(scheduleForm.end_time) // 修改参数名称
            };
                ...
                return prevDoctors.map(doctor => {
                    if (doctor.id === doctorId) {
                        ...
                        const newSchedule = {
                            ...
                            start_time: formatDateTime(scheduleForm.start_time), // 保持与后端一致
                            end_time: formatDateTime(scheduleForm.end_time) // 保持与后端一致
                        };

                        // 更新医生的排班列表
                        const updatedSchedules = doctor.schedules.length > 0 ? [newSchedule] : [...doctor.schedules, newSchedule];
    ...
    const handleEditScheduleClick = (doctor: Doctor) => {
        ...
        if (currentSchedule) {
            ...
            const formatDateTimeForInput = (mysqlDateTime: string) => {
                // 使用 moment 转换为本地时间
                const localDateTime = moment.utc(mysqlDateTime).local();

                // 格式化为YYYY-MM-DDTHH:mm
                const formatted = localDateTime.format('YYYY-MM-DDTHH:mm');
                console.log(`格式化日期时间: ${mysqlDateTime} -> ${formatted}`);
                return formatted;
            };

            const formData: ScheduleForm = {
                appointment_price: String(currentSchedule.appointment_price), // 转换为字符串类型
    ...
    const handleAddDoctor = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        // 3. 获取最新的排班信息
        // 使用正确的接口地址
        const latestScheduleRes = await fetch(`${apiUrl}/doctorSchedules?doctorId=${doctorData.doctorId}`);
        ...
        if (latestScheduleRes.ok) {
            ...
            console.error('获取最新的排班信息失败:', latestScheduleRes.status, latestScheduleRes.statusText);
            // 可以选择抛出错误或给出提示信息
            // throw new Error(`获取最新的排班信息失败: ${latestScheduleRes.status} ${latestScheduleRes.statusText}`);
        }
...
const formatTimeRange = (startTime: string, endTime: string) => {
    ...
    // 格式化为中文日期格式，例如：2025年06月04日
    const startDate = start.format('YYYY年MM月DD日');
    const endDate = end.format('YYYY年MM月DD日');
